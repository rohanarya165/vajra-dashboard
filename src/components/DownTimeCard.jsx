...
const DowntimeCards = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
      {cards.map((card, i) => (
        <div key={i} className="bg-white rounded-2xl p-4 shadow-sm flex flex-col justify-between">
          ...
          {card.time ? (
            ...
            <>
              <h2 className="text-sm mt-4 font-medium text-gray-800 flex items-center justify-between">
                {card.title}
                {card.tag && (
                  <span className="ml-2 bg-blue-100 text-xs text-blue-600 px-2 py-1 rounded-lg">{card.tag}</span>
                )}
              </h2>
              ...
              <div className="flex items-center gap-1 mt-3">
                {card.team.map((t, idx) => (
                  <span
                    key={idx}
                    className={`w-6 h-6 rounded-full bg-pink-400 text-white text-xs flex items-center justify-center`}
                    style={{ backgroundColor: ['#ec4899', '#3b82f6', '#10b981', '#f59e0b', '#e5e7eb'][idx] }}
                  >
                    {t}
                  </span>
                ))}
